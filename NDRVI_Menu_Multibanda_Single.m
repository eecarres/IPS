...
%             end 
         ...
         % Calculamos NDVI 800 si se ha seleccionado la opción
            ...
            % Calculamos FB1 si se ha seleccionado la opción
            if get(handles.chkFB1,'Value')==1       
                if  isempty(R670) || isempty(R730) || isempty(R550)  || isempty(R450)
                    msgbox('No hay información suficiente para calcular el índice: Comprueba las bandas de entrada','Error calculando índice FB1 Hemav');
                    return;
                else
                        ...
                        dataImProcesada(:,:,numBandas)=(FB1)*1000;
                                       if i==1
                                           maxBanda=max(max(dataImProcesada(:,:,numBandas)));
                                                             minBanda=min(min(dataImProcesada(:,:,numBandas)));
                                    fprintf(fileID,'%s \r\n',['Banda número',' ',num2str(numBandas),': FB1 Hemav','   Rango:(',num2str(maxBanda),' - ',num2str(minBanda),')']);
                                       end
                end
            end
            
            % Calculamos FB2 si se ha seleccionado la opción
            if get(handles.chkFB2,'Value')==1       
                if  isempty(R670) || isempty(R780) || isempty(R550)  || isempty(R450)
                    msgbox('No hay información suficiente para calcular el índice: Comprueba las bandas de entrada','Error calculando índice FB2 Hemav');
                    return;
                else
                        ...
                        dataImProcesada(:,:,numBandas)=(FB2)*1000;
                                       if i==1
                                           maxBanda=max(max(dataImProcesada(:,:,numBandas)));
                                                             minBanda=min(min(dataImProcesada(:,:,numBandas)));
                                    fprintf(fileID,'%s \r\n',['Banda número',' ',num2str(numBandas),': FB2 Hemav','   Rango:(',num2str(maxBanda),' - ',num2str(minBanda),')']);
                                       end
                end
            end
