function [varargout]= TCARI_OSAVI_Multibanda (R710,R670,R550,R780,iteracion,chkImagenes,chkProceso,tipoCmap,boolHist,status_cuad,cuad_div,rgb_g_limits,auxiliar_limits,status_suelo,check_aux,varargin)
...
if nargin==19% En este caso entra una imagen multibanda, de modo que en lugar de los paths se pasan las matrices
%msgbox(num2str(nargin));
LK_R710= ND_Banda_LK (chkImagenes,chkProceso,R710,iteracion,varargin{1}); 
LK_R670= ND_Banda_LK (chkImagenes,chkProceso,R670,iteracion,varargin{2}); 
LK_R550= ND_Banda_LK (chkImagenes,chkProceso,R550,iteracion,varargin{3}); 
LK_R780= ND_Banda_LK (chkImagenes,chkProceso,R780,iteracion,varargin{4}); 
     
elseif nargin ==15
LK_R710= ND_Banda_LK (chkImagenes,chkProceso,R710,iteracion); 
LK_R670= ND_Banda_LK (chkImagenes,chkProceso,R670,iteracion); 
LK_R550= ND_Banda_LK (chkImagenes,chkProceso,R550,iteracion); 
LK_R780= ND_Banda_LK (chkImagenes,chkProceso,R780,iteracion); 
...
C=LK_R550;
D=LK_R780;
 ima=3*(    (A-B) - 0.2*((A-C).*(A./B))    )  ./  (1.16*(D-B)./(D+B+0.16));
...
if nargin==19
  ...
  NDRVI_im_save(name_im,'TCARI',chkProceso,ima,ColMap); 
end
